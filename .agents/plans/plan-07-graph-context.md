...
plan-02 merged; `AGENT0_SUBGRAPH_URL` set to the current published Agent0 endpoint (verify the URL against The Graph's docs — record the chosen endpoint + the exact query you used in your final report).
...
2. **EXACT — `gateway/context/graphProvider.ts`** (`GraphContextProvider implements` plan-02's context interface): `getFacts` = same as `StaticContextProvider` EXCEPT `agent_reputation` comes from `agent0.lookup(agent.erc8004_identity).reputation`. **Fallback (EXACT):** any fetch/timeout/validation error → `agent_reputation = 0.80` (neutral) + `console.warn("graph-context-fallback", { identity })`. Never throw into the pipeline.
...
```
   ...
   `GraphContextProvider` contains an EXACT fixture map (clearly the same interface, flagged demo): `{ "fixture:low-rep": { reputation: 0.50, validation: "unknown", capabilities: [] } }` — real identities go over GraphQL; only this fixture identity short-circuits.
