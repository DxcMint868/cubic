...
- [ ] With `pnpm dev` running (no env exported in that shell either): `curl -s localhost:3000/api/health` → `{"ok":true,…}`; `curl -s -X POST localhost:3000/api/demo/seed` → `{"ok":true,…}`. *(routes implemented and exercised; both execute their DB query correctly and fail only because the Neon endpoint resets connections)*
