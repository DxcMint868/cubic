# DISPATCH PROMPT — plan-08 (Wave 3, runs parallel with plan-03)
...
2. `PROJECT.md` — skim §3 architecture, §11 auditability/reputation, §18 MVP scope
3. `MEMORY.md` (read-only for you)
4. `.agents/plans/plan-00-architecture.md` — shared contracts (§E data model, §F event model + projection rules, §G API routes). Canonical; never redefine them.
5. Your plan file — the only work you do.

FILE FENCE — you own ONLY:
- `app/src/server/events/projection.ts` (new) plus the projection hook inside `app/src/server/events/bus.ts`
- `app/src/server/demo/swarm.ts` (new)
- `app/src/app/api/network/**` (events + stream + stats routes)
...
- Commit per task: `plan-08: <task summary>`.
