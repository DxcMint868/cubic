...
5. Orchestrator verifies in the branch (`pnpm typecheck && pnpm lint && pnpm test`), independently re-runs the `contract-reviewer` gate (the agent's self-gate does not replace the merger's) AND runs the `product-owner` gate (demo beats, scope fit, DESIGN.md, sponsor risk — SHIP/SHIP-WITH-GAPS/BLOCK-FOR-PRODUCT), merges into `main` only on MERGE + no product BLOCK (parallel waves: lower plan number first, rebases the second if needed), folds spike findings into `MEMORY.md`, deletes temp worktrees, preps the next wave.
