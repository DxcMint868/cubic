...
- [x] Orchestration set up: `.agents/dispatch/` contains `RUNBOOK.md` + verbatim dispatch prompts plan-01…plan-10 (fences, git discipline, test-tenant rules, done criteria). Waves: W1 01 solo → W2 02 solo → W3 03∥08 → W4 04∥07 → W5 05∥06 → W6 09 solo → W7 10 solo. Worktree per plan REMOVED (simplified 2026-09-10): solo waves (W1 01, W2 02, W6 09, W7 10) build right in the main repo on branch `plan-XX`; parallel waves (W3 03∥08, W4 04∥07, W5 05∥06) get temporary sibling worktrees (`../cubic-pXX`, created per wave off latest main, deleted after merge) — the standing `cubic-p01` worktree was deleted. Merge order within a wave = lower plan number first. Agents never edit MEMORY.md (merger folds their `## Spike findings` into it); plan-10 is the sole final-update exception.
- [ ] **W1 plan-01 IN PROGRESS**: main rebased onto `origin/main` 2026-09-10 (upstream landing work intact + our 6 orchestration commits replayed clean, zero conflicts); env received (`app/.env.local` present + ignored); branch `plan-01` cut fresh from rebased main. Launch prompt: `.agents/dispatch/plan-01.md` (paste verbatim into an agent running in this repo dir). Awaiting "plan-01 done" + final report.
