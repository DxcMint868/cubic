...
- [x] Orchestration set up: `.agents/dispatch/` contains `RUNBOOK.md` + verbatim dispatch prompts plan-01…plan-10 (fences, git discipline, test-tenant rules, done criteria). Waves: W1 01 solo → W2 02 solo → W3 03∥08 → W4 04∥07 → W5 05∥06 → W6 09 solo → W7 10 solo. Worktree per plan REMOVED (simplified 2026-09-10): solo waves and parallel waves alike get a sibling worktree per active plan (`../cubic-pXX`, branch `plan-XX`, created per wave off latest main with deps installed + `.env.local` copied in, deleted after merge) — agents never run in the home repo; home `main` stays pristine (merges only). Merge order within a wave = lower plan number first. Agents never edit MEMORY.md (merger folds their `## Spike findings` into it); plan-10 is the sole final-update exception.
- [ ] **W1 plan-01 IN PROGRESS**: worktree `../cubic-p01` (branch `plan-01`, deps installed, `.env.local` copied in). Launch: open `../cubic-p01` in the agent tool, paste `.agents/dispatch/plan-01.md` verbatim. Awaiting "plan-01 done" + final report.
