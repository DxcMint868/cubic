...
|---|---|---|---|---|
| A | Landing page, top→bottom scroll | 0:00–0:25 | 1 problem | story |
| B | Console → Agents | 0:25–0:45 | 2 cast | story |
| C | Chat: free-text PR read → ALLOW | 0:45–1:10 | 2 gateway | — |
| D | Chat: 402 → purchase → Payments → HashScan | 1:10–1:45 | 3–4 x402 | **Hedera** |
| E | Two-tab: merge → live approval → wallet sign → execution | 1:45–2:20 | 4–5 approvals | **Hedera** (HCS anchor of signature) |
| F | Chat: injected secret read + overbudget → DENY ×2 | 2:20–2:40 | 6 attack | story |
| G | lab-1 escalate → Graph playground + QuickNode page | 2:40–3:05 | 6 reputation | **The Graph** |
| H | Verify-on-HCS page → HashScan topic | 3:05–3:25 | 7 anchoring | **Hedera** |
| I | Network surface → closer | 3:25–3:40 | 7 network | story |
...
2. Footage C, D, F (chat takes) — several takes each, keep the best.
3. Footage E (two-tab) — needs the chat take's merge escalation live; record
   both tabs in one screen capture.
4. Footage G (reputation) — live web, load pages BEFORE rolling.
5. Footage H (HCS verify) — right after E so signatures are fresh in the window.
6. Footage B, I (console + network) — anytime.
7. Footage A (landing scroll) — anytime; slowest scroll you can tolerate.
